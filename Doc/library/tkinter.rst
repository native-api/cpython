...
.. class:: Variable(master=None, value=None, name=None)
   ...
   *name* is an optional Tcl name for the variable. For instance, this
   ...
   .. method:: Variable.trace_add(mode, callback)
      ...
      *callback* must be a function which is called when the variable is
      ...
      used in :meth:`Variable.trace_remove`.
      
   .. method:: Variable.trace_remove(mode, cbname)
      ...
      *mode* is one of ``"read"``, ``"write"``, ``"unset"``, or a list or
      tuple of these.  Must be same as were specified in :meth:`Variable.trace_add`.
      
      *cbname* is the callback name that was returned by :meth:`Variable.trace_add`.

   .. method:: Variable.trace_info()
   
      Delegates to `trace info variable <https://tcl.tk/man/tcl8.6/TclCmd/trace.htm#M26>`_.
      
      Returns a list of tuples.
      
   .. method:: Variable.trace_variable(mode, callback)
   .. method:: Variable.trace_vdelete(mode, callback)
   .. method:: Variable.trace_vinfo(mode, callback)
   
      Deprecated counterparts to :meth:`Variable.trace_add`, :meth:`Variable.trace_remove` and
      :meth:`Variable.trace_info`, correspondingly.
      
      Delegate to likewise deprecated ``trace variable``, ``trace vdelete`` and ``trace vinfo``
      Tk commands.
...
.. :function:: getboolean(value)
   ...
   ?Deprecated and unsupported. The corresponding methods of :mod:`_tkinter`
...
.. class:: Tk(screenName=None, baseName=None, className='Tk', useTk=1, sync=0, use=None)
   ...
   *sync* and *use* add
   ...
   in ``$HOME`` or current directory is ``$HOME`` is unset, with names
   ...
   A callback that is called if Python payload in an event handler
   raised an exception. The default implementation prints the
...
Unless specified otherwise, all Tkinter calls that map to Tcl/Tk calls convert
